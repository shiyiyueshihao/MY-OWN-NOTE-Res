---
title: Docker_Nginx
date: 2026-05-21T02:12:00.000Z
updated: 2026-05-21T02:14:00.000Z
---
...
```plain text
打开 docker 进入 container内部 打开终端 输入 docker pull 进行初始化
```
...
```plain text
打开浏览器 输入地址 localhost  有没有 nginx 的欢迎语  有 则成功 无则失败
```
...
```plain text
nginx 启动成功 则可以把刚才创建的 html 文件内的文件全部删除 替换成你 打包的 dist内部的所有文件
```
